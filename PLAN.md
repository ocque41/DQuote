...
- Reintroduce a marketing homepage with hero, features, and CTA under a dedicated public layout.
- Ensure marketing and docs routes stay outside auth middleware while retaining authenticated CTA for signed-in users.

### Acceptance Criteria

- [x] Visiting `/` renders the marketing homepage with navbar/footer and does not trigger auth middleware.
- [x] `/docs` (and other marketing routes) remain publicly accessible while signed-in users see a "Dashboard" CTA.

## Item 2: Fix Neon Auth sign-in flows

- Embed Neon Auth SignIn/SignUp components inside the existing shadcn login-04 shell and surface errors gracefully.
- Validate StackProvider wiring plus env vars so successful auth redirects to `/dashboard` without client exceptions.

### Acceptance Criteria

- [x] `/login` and `/signup` authenticate against Neon Auth and redirect to `/dashboard` on success.
- [x] Auth errors log server-side diagnostics while the UI shows a friendly toast/message without leaking sensitive data.

## Item 3: Structure app shell + theme consistency

- Move dashboard/quotes into an `(app)` route group with middleware restricted to those paths.
- Standardize green theme spacing/containers (sidebar width, cards, typography) and keep the Quote Terminal scaffold intact.

### Acceptance Criteria

- [x] `/dashboard` and `/quotes` share consistent containers/spacing with sidebar width tokens applied via CSS vars.
- [x] Unauthenticated visitors hitting `/dashboard` or `/quotes` are redirected to `/login`, while the Quote Terminal stub remains available post-auth.
